...
To make this clear, we will answer these three question one by one. By end this chapter, we will see how variable, feature and random variable are closely connected. 
...
if we measure many time, we collect multiple data points.
...
Each ovservation can be seen as a point, if we mwasure two variable(e.g., height & weight), we can plot them. 
...
Before we measure, wo do not know the value, if we randomly choose a person, their height is unkonwn.
...
A **random variable** describes a quantity whose value is unkown before we observe it. 
...
In practice, we usually collect more than one observation. If we measure height and weight for many people, each person gives us one pair of values. These paris can be written as points on a scatter plot. 
